...
| Model records | Partial | Active model and an observed profile can be synchronized. The scar Hugging Face cache is enumerated through a bounded read-only mount and shown in the dashboard. Metadata/downloads and removal safeguards remain missing. |
...
| Automated tests | Partial | 26 pytest and 6 BATS tests cover backend/dashboard behavior plus image pinning, restart policy, read-only cache mounting, AMD SMI mounting, and shell safety. Broader browser/matrix/security coverage remains. |
